...
<div class="user">
   ...
   <form action="/users/processCreateUser" method="POST">
        ...
        <div>
            <input type="text" name="first_name" placeholder="Enter your first name" class="<?= session()->has('signup_error_first_name') ? "error" : ""; ?>" value="<?= @session()->get('signup_value_first_name'); ?>">
            <div class="invalid-feedback"><?= @session()->get('signup_error_first_name'); ?></div>
        </div>
        <div>
            <input type="text" name="last_name" placeholder="Enter your last name" class="<?= session()->has('signup_error_last_name') ? "error" : ""; ?>" value="<?= @session()->get('signup_value_last_name'); ?>">
        ...
        <div>
            <input type="password" name="confirm_password" placeholder="Confirm your password" class="<?= session()->has('signup_error_confirm_password') ? "error" : ""; ?>" value="<?= @session()->get('signup_value_confirm_password'); ?>">
            <div class="invalid-feedback"><?= @session()->get('signup_error_confirm_password')?></div>
        </div>
        <input type="hidden" name="<?= csrf_token(); ?>" value="<?= csrf_hash(); ?>">
        <input type="submit" value="Sign up">
        <p>Do you have already an account? <a href="/signin">Sign in</a></p>
   </form>
